...
from hmr.domain import ReaderKnowledge
...
class TestPromptRenderInLLMService:
    ...
    def test_all_prompt_file_could_be_loaded(self, prompt_loader):
        prompt_list = prompt_loader.list_all()
        for prompt in prompt_list:
            assert isinstance(prompt_loader.prompts[prompt], Path), f"Prompt file {prompt} is not pointing to a Path object"
            loaded = prompt_loader.load(prompt)
    ...
    def test_prompt_render_in_llmservice(self, mock_llm_client, prompt_loader):
        ...
        llm_service.merge_answers(question="question", answers=["answer"])
